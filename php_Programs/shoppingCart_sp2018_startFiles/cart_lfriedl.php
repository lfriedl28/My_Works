...
<html>
    ...
    <body>
		...
		// No sanitization required for this part
        ...
        foreach ($_POST as $productID => $productQty) {
			...
			if(is_numeric($productQty) && number_format($productQty, 0) > 0 && !isset($remove[$productID])){
                
                $cart[$productID] = number_format($productQty, 0);
            
            } else if(is_numeric($productQty) && (number_format($productQty, 0) == 0 || isset($remove[$productID]))) {
		...
		foreach ($cart as $prodID => $productQty) {
            $prodIDStr .= "$prodID,";
        ...
        if (!$_SESSION['numItems']) {   // cart is empty
            ...
            $prodIDStr = substr($prodIDStr, 0, strlen($prodIDStr) - 1);
            ...
            $query = "SELECT * FROM products
                      ...
                      ORDER BY prodid";
            $error = "Error fetching cart contents";
